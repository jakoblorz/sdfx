...
func (s *Scene) World() Hitable {
	return s.world
}

func NewScene(width, height int, raysPerPixel []int, camera Camera, world Hitable) *Scene {
	return &Scene{
		width:        width,
		height:       height,
...
func trace(r *Ray3, world Hitable, depth int) Color {
	...
	if hit, hr := world.Hit(r, 0.001, math.MaxFloat64); hit {
		...
		if wasScattered, attenuation, scattered := hr.material.Scatter(r, hr); wasScattered {
			return attenuation.Mult(trace(scattered, world, depth+1))
		} else {
			return Black
		}
	}
